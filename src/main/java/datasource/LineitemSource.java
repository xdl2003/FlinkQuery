...
public class LineitemSource implements SourceFunction<DataEvent<Lineitem>> {
    private final String filePath;
    private volatile boolean isRunning = true; // 控制 run 方法的循环

    public LineitemSource(String filePath) {
        this.filePath = filePath;
    }

    @Override
    public void run(SourceFunction.SourceContext<DataEvent<Lineitem>> ctx) throws Exception {
        ...
        try {
            ...
            while (isRunning && (event = parser.nextEvent()) != null) {
                ...
                if (event.getTableName().equals("lineitem")) {
                    ctx.collect((DataEvent<Lineitem>) event);
                }
            }
        } catch (Exception e) {
